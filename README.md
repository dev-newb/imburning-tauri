...
Confirmed weekly resets play the choir after a second fresh provider reading confirms the change. Five-hour reset indications stay silent and are logged with phase `suppressed` because they do not independently restore the weekly quota. A cached reading or redraw cannot confirm a reset. Temporary zeros, missing bank counts and account changes stay quiet; the alert can arrive one refresh after the first reset reading.
